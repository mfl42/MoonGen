...
if [[ -z "${VMOONGEN_DPDK_ROOT:-}" ]]; then
	for _cand in \
		"$ROOT/../dpdk-stable" \
		"$HOME/Projects/dpdk-stable" \
		"$HOME/dpdk-stable"
	do
		if [[ -d "$_cand" ]]; then
			export VMOONGEN_DPDK_ROOT="$_cand"
			break
		fi
	done
fi
...
export VMOONGEN_DPDK_ROOT="${VMOONGEN_DPDK_ROOT:-$HOME/Projects/dpdk-stable}"
export VMOONGEN_DPDK_GIT_URL="${VMOONGEN_DPDK_GIT_URL:-git@github.com:mfl42/dpdk-stable.git}"
export VMOONGEN_DPDK_GIT_BRANCH="${VMOONGEN_DPDK_GIT_BRANCH:-24.11}"
export VMOONGEN_DPDK_UPSTREAM_URL="${VMOONGEN_DPDK_UPSTREAM_URL:-https://github.com/DPDK/dpdk-stable.git}"
...
export VMOONGEN_VPP_BUILD_RUNTIME="${VMOONGEN_VPP_BUILD_RUNTIME:-podman}"
export VMOONGEN_VPP_BUILD_IMAGE="${VMOONGEN_VPP_BUILD_IMAGE:-localhost/vmoongen-vpp-build:ubuntu24.04}"
export VMOONGEN_VPP_BUILD_JOBS="${VMOONGEN_VPP_BUILD_JOBS:-4}"
export VMOONGEN_VPP_BUILD_CONTEXT="${VMOONGEN_VPP_BUILD_CONTEXT:-$ROOT}"
export VMOONGEN_VPP_BUILD_CONTAINERFILE="${VMOONGEN_VPP_BUILD_CONTAINERFILE:-$ROOT/containers/Containerfile.vpp-build}"
export VMOONGEN_VPP_BUILD_MOUNT_ROOT="${VMOONGEN_VPP_BUILD_MOUNT_ROOT:-/workspace/vmoongen}"
export VMOONGEN_VPP_BUILD_MOUNT_VPP="${VMOONGEN_VPP_BUILD_MOUNT_VPP:-/workspace/vpp}"
export VMOONGEN_VPP_BUILD_MOUNT_DPDK="${VMOONGEN_VPP_BUILD_MOUNT_DPDK:-/workspace/dpdk-stable}"
export VMOONGEN_VPP_SYSTEM_DPDK_BUILD_DIR="${VMOONGEN_VPP_SYSTEM_DPDK_BUILD_DIR:-$VPP_ROOT/build-root/build-system-dpdk}"
export VMOONGEN_VPP_SYSTEM_DPDK_PREFIX="${VMOONGEN_VPP_SYSTEM_DPDK_PREFIX:-$VPP_ROOT/build-root/install-system-dpdk}"
...
if [[ -z "${VMOONGEN_LUAJIT_BIN:-}" ]]; then
	if command -v luajit >/dev/null 2>&1; then
		VMOONGEN_LUAJIT_BIN="$(command -v luajit)"
	elif [[ -x "$ROOT/libmoon/deps/luajit/src/luajit" ]]; then
		VMOONGEN_LUAJIT_BIN="$ROOT/libmoon/deps/luajit/src/luajit"
	elif [[ -x "$ROOT/build/luajit-prefix/src/luajit/src/luajit" ]]; then
		VMOONGEN_LUAJIT_BIN="$ROOT/build/luajit-prefix/src/luajit/src/luajit"
	else
		VMOONGEN_LUAJIT_BIN=""
	fi
fi
export VMOONGEN_LUAJIT_BIN

export DPDK_DEVBIND="${DPDK_DEVBIND:-$ROOT/libmoon/deps/dpdk/usertools/dpdk-devbind.py}"

vmoongen_have_build_runtime() {
	command -v "$VMOONGEN_VPP_BUILD_RUNTIME" >/dev/null 2>&1
}

vmoongen_system_dpdk_pkgconfig_path() {
	local paths=()
	local candidate

	for candidate in \
		"$VMOONGEN_VPP_SYSTEM_DPDK_PREFIX/lib/pkgconfig" \
		"$VMOONGEN_VPP_SYSTEM_DPDK_PREFIX/lib64/pkgconfig"
	do
		[[ -d "$candidate" ]] && paths+=("$candidate")
	done
...
vmoongen_have_luajit() {
	[[ -n "${VMOONGEN_LUAJIT_BIN:-}" && -x "$VMOONGEN_LUAJIT_BIN" ]]
}
